...
void runFractal(infoType * fractalInfo);
...
int main (int argc, char *argv[]){
	...
	//HABRIA QUE PASAR AL DISPLAY COMO PARAMETRO, ESPERAR A QUE LOS CHICOS CAMBIEN SUS FUNCIONES, PEDIRLES!

	if( ( parseCmdLine(argc, argv, functionPtr, &userInput) > 0 ) && verifiedData(&userInput)){
		//crear display!!!
		runFractal(&userInput);
	}
	else
...
void runFractal(infoType * fractalInfo){
	...
	{
		recursive_polygon(fractalInfo);
	}
	else if( !strcmp(userInput->type, "MANDELBROT") ){

		if( MandelAl(fractalInfo) )
			printf("hubo un error a la hora de dibujar el fractal! Pruebe con otros Inputs.");
	}
}
...
int parseCallback(char *key, char *value, void *userInput)
{
	...
	if (key != NULL){
		...
		if(!strcmp(key, "TYPE")){
			...
			//verifico que N no haya sido inicializada por usuario y pertenezca a (0, 100]	
